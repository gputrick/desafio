 ...
 *                 "ProdutoCategoriaProdutoSession"
 *
 * @ejbgen:jndi-name remote =
 *                   "ejb.DesafioEjbs.ProdutoCategoriaProdutoSessionRemoteHome"
 *
 * @ejbgen:file-generation remote-class = "true" remote-class-name =
 *                         "ProdutoCategoriaProdutoSessionRemote" remote-home =
 *                         "true" remote-home-name =
 *                         "ProdutoCategoriaProdutoSessionHome" local-class =
 *                         "false" local-class-name =
 *                         "ProdutoCategoriaProdutoSessionLocal" local-home =
 *                         "false" local-home-name =
 *                         "ProdutoCategoriaProdutoSessionLocalHome"
 */
public class ProdutoCategoriaProdutoSession extends GenericSessionBean implements SessionBean {
	...
	private void inicializa(ProdutoCategoriaProdutoVO produtoCategoriaProdutoVO) throws DesafioException, EJBException {
		...
		try {
			if ((produtoCategoriaProdutoVO != null) && (produtoCategoriaProdutoVO.getId_produto() > 0)
				...
				ProdutoCategoriaProdutoBeanPK produtoCategoriaProdutoPK = new ProdutoCategoriaProdutoBeanPK(
						new Long(produtoCategoriaProdutoVO.getId_categoria_produto()),
	...
	/**
	 * Método obtém os dados primitivos do produto atravás da primary key
	 * 
	 * @param produtoVO
	 * @throws DesafioException
	 */
	...
	/**
	 * Método que obtem os dados primitivos do objeto pela primary key
	 * 
	 * @param categoriaProdutoVO
	 * @throws DesafioException
	 */
	private CategoriaProduto inicializaCategoriaProduto(ProdutoCategoriaProdutoVO produtoCategoriaProdutoVO)
			throws DesafioException {
		...
		try {
			...
			if ((produtoCategoriaProdutoVO != null) && (produtoCategoriaProdutoVO.getId_categoria_produto() > 0)) {

				categoriaProduto = categoriaProdutoHome
						.findByPrimaryKey(new Long(produtoCategoriaProdutoVO.getId_categoria_produto()));
			...
			throw new DesafioException(
					"Desafio021 - ProdutoCategoriaProduto: CategoriaProduto não encontrado ou não existente");
		} catch (Exception e) {
			throw new EJBException(e);
		}
	 ...
	 */
	public void eliminarProdutoCategoriaProduto(ProdutoCategoriaProdutoVO produtoCategoriaProdutoVO)
			throws DesafioException {
		try {
	 ...
	 * @ejbgen:remote-method transaction-attribute="Required"
	 */
	public ProdutoCategoriaProdutoVO obterProdutoCategoriaProduto(ProdutoCategoriaProdutoVO produtoCategoriaProdutoVO)
			throws DesafioException {
		this.inicializa(produtoCategoriaProdutoVO);
	...
	public static ProdutoCategoriaProdutoSessionHome getRemoteHome() throws NamingException {
		return (ProdutoCategoriaProdutoSessionHome) ServiceLocator.getInstance().getRemoteHome(getRemoteJndiName(),
				ProdutoCategoriaProdutoSessionHome.class);
	}
	 ...
	 * @ejbgen:remote-method transaction-attribute="Required"
	 */
	public ProdutoCategoriaProdutoVO atualizarProdutoCategoriaProduto(
			ProdutoCategoriaProdutoVO produtoCategoriaProdutoVO) throws DesafioException, NamingException {
		try {
			this.inicializa(produtoCategoriaProdutoVO);
