...
<body>
	<div class="container">
		...
		<netui:anchor action="acIrParaVisualizarCategoriaPai">
			<netui:label tagId="nome_categoria_pai"
				value="{pageFlow.categoriaProdutoFormBean.categoriaProdutoVO.categoriaPaiVO.nome_categoria_produto}" />
		</netui:anchor>
		...
		<netui-data:grid dataSource="{pageFlow.listaSubCategoria}"
			...
			<netui-data:columns filterAction="acFiltroListaSubCategoriaProduto"
				sortAction="acOrdenaListaSubCategoriaProduto">
				<netui-data:anchorColumn action="acSelecionaSubCategoria"
					styleClassPrefix="left" addRowId="true" title="Nome Sub-categoria"
					name="NOME_CATEGORIA_PRODUTO" />
				<netui-data:anchorColumn action="acSelecionaSubCategoria"
					styleClassPrefix="right" addRowId="true" title="&nbsp;"
					src="/DesafioWeb/resources/images/eye-icon.png" width="16" />
			</netui-data:columns>
		</netui-data:grid>
		...
		<netui:form tagId="formEliminarCategoriaProduto"
			...
			<netui:hidden tagId="codigo"
				dataSource="{pageFlow.categoriaProdutoFormBean.categoriaProdutoVO.id_categoria_produto}" />
		</netui:form>
		<script type="text/javascript">
			//confirma a exclusao de um registro  
			function confirmaExclusao() {
				var forme = document.forms[getNetuiTagName(
						"formEliminarCategoriaProduto", this)];
